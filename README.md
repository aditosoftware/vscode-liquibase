...
- "Use any of the recent contexts": These will give you a second dialog where all the cached contexts are loaded. You should use this option, if you want to use contexts and these have not changed from any previous cached contexts. This option will be only visible, if there are cached contexts.

Both "Load all contexts from the changelog file" and "Use any of the recent contexts" will not use any contexts for your command, if there is no context selected.
